...
object LeverAura: Module(
    ...
    class Lever (val coords: BlockPos, var lastClick: Long = 0L, var lastCompletedClick: Long = 0L)
    ...
    private val levers = listOf(
        Lever(BlockPos(106, 124, 113)),
        Lever(BlockPos(94, 124, 113)),
        Lever(BlockPos(23, 132, 138)),
        Lever(BlockPos(27, 124, 127)),
        Lever(BlockPos(2, 122, 55)),
        Lever(BlockPos(14, 122, 55)),
        Lever(BlockPos(84, 121, 34)),
        Lever(BlockPos(86, 128, 46)),
        Lever(BlockPos(62, 133, 142)),
        Lever(BlockPos(62, 136, 142)),
        Lever(BlockPos(60, 135, 142)),
        Lever(BlockPos(60, 134, 142)),
        Lever(BlockPos(58, 136, 142)),
        Lever(BlockPos(58, 133, 142))
    )
        ...
        when (packet) {
            ...
            is S22PacketMultiBlockChange -> {
                for (block in packet.changedBlocks) {
                    handleChangedBlock(block.pos)
                }
            }
        }
    }
    private fun handleChangedBlock(pos: BlockPos){
        val lever = levers.firstOrNull { it.coords == pos } ?: return
        val state = getBlockStateAt(pos)
        if (state.block === Blocks.lever) {
            lever.lastCompletedClick = lever.lastClick
            return
        }
    }
}