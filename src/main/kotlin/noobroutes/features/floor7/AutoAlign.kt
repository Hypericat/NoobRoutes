...
import noobroutes.utils.flooredVec
...
object AutoAlign: Module(
    ...
    /**
     * Based on odins solver
     */

    private val blockWrong by BooleanSetting(
        "Block Wrong Clicks",
        false,
        description = "Blocks wrong clicks, shift will override this."
    )
    ...
    fun onTick(event: TickEvent.ClientTickEvent) {
        if (event.isNotEnd) return

        clicksRemaining.clear()
        if ((mc.thePlayer?.positionVector?.squareDistanceTo(Vec3(0.0, 120.0, 77.0)) ?: return) > 1000) return

        val currentFrameRotations = getFrames()

        possibleSolutions.forEach { arr ->
            for (i in arr.indices) {
                if ((arr[i] == -1 || currentFrameRotations[i].first == -1) && arr[i] != currentFrameRotations[i].first) return@forEach
            }

            targetSolution = arr

            for (i in arr.indices) {
                val clicksNeeded = calculateClicksNeeded(currentFrameRotations[i].first, arr[i]).takeIf { it != 0 } ?: continue
                val frameEntity = currentFrameRotations[i].second ?: return

                if (frameEntity.positionVector.squareDistanceTo(mc.thePlayer.getPositionEyes(1f)) > 25) continue

                val auraEntity = AuraManager.EntityAura(frameEntity, C02PacketUseEntity.Action.INTERACT)
                repeat(clicksNeeded) {
                    AuraManager.clickEntity(auraEntity)
                }
                frameEntity.setItemRotation(arr[i])
                clicksRemaining[i] = 0
                return
            }
        }
    ...
    private fun getFrames(): List<Pair<Int, EntityItemFrame?>> {
        val itemFrames = mc.theWorld?.loadedEntityList?.mapNotNull {
            if (it is EntityItemFrame && it.displayedItem?.item == Items.arrow) it else null }?.takeIf { it.isNotEmpty() } ?: return List(25) { -1 to null }

        val frameMap = itemFrames.associateBy { it.positionVector.flooredVec().toString() }

        return (0..24).map { index ->
            val framePos = getFramePositionFromIndex(index).toString()
            val frame = frameMap[framePos]
            (frame?.rotation ?: -1) to frame
        }
    }
